...
Function DbInfLnkDt(Optional D As Database) As Dt
Dim T, Dry(), C$
...
Function DbInfStruDt(Optional D As Database) As Dt
Dim T, Dry()
For Each T In DbTny(D)
...
Function DbInfTblFDt(Optional D As Database) As Dt
Dim T, Dry()
For Each T In DbTny(D)
...
Sub DbTBrw(T, Optional D As Dao.Database)
DtBrw TblDt(T, D)
End Sub
